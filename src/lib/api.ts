...
export function suggestBestPostTime(niche?: string): BestTimeSuggestion {
  ...
  const now = new Date();
  if (n.includes('game') || n.includes('gaming')) return nextOccurrence(4, 19, 'Gaming audiences are most active weekday evenings');
  if (n.includes('finance') || n.includes('business') || n.includes('b2b')) return nextOccurrence(3, 12, 'Finance/B2B audiences engage most over lunch');
  if (n.includes('fitness') || n.includes('workout')) return nextOccurrence(3, 7, 'Fitness content performs best in morning');
  return nextOccurrence(4, 15, 'Thursday afternoon is a strong general benchmark');
  ...
  function nextOccurrence(targetDay: number, hour: number, reason: string): BestTimeSuggestion {
    ...
    return {
      ...
      dayOfWeek: targetDay, hour, reason,
    };
  }
}
...
export async function scheduleAutoPublish(videoId: string, youtubeChannelId: string, scheduledFor: Date): Promise<void> {
  ...
  const { error } = await supabase.from('scheduled_publishes').insert({
    user_id: user.id, video_id: videoId, youtube_channel_id: youtubeChannelId,
...
export async function chatWithAgent(message: string, history: { role: 'user' | 'assistant'; content: string }[]): Promise<string> {
  const settings = await getUserSettings();
  const apiKey = settings?.gemini_api_key;
  if (!apiKey) throw new Error('NO_API_KEY');
...
export async function createShortsFromLink(videoURL: string): Promise<Short[]> {
  ...
  const { data: parentVideo, error: videoError } = await supabase
    .from('videos')
    .insert([{ file_path: videoURL, status: 'ready', user_id: user.id, title: 'Processing Long Video...' }])
    .select().single();
  ...
  const { data: shorts, error: shortsError } = await supabase.from('shorts').select('*').eq('parent_video_id', parentVideo.id).order('created_at', { ascending: true });
